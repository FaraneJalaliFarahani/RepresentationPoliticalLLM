...
if __name__ == "__main__":
    ...
    for model_name in [args.model]:
        ...
        pickle.dump(statements, open(f'./results_replication/{model_name.replace("/", "_")}_{args.axis}_{args.prob}.pkl', 'wb'))
    ...
    for model_name in [args.model]: #
        ...
        statements = pickle.load(open(f'./results_replication/{model_name.replace("/", "_")}_{args.axis}_{args.prob}.pkl', 'rb'))
        labels = np.array(df[args.axis].astype(float))
        features = extract_attention_head_activations(model, statements)
        pickle.dump((features, labels), open(f"./results_replication/{model_name.replace('/','_')}_{args.axis}_{args.prob}{suffix}_features.pkl", 'wb'))
    ...
    for model_name in [args.model]:
        ...
        pickle.dump(performance, open(f"./results_replication/{model_name.replace('/','_')}_{args.axis}_{args.prob}{suffix}_performance.pkl", 'wb'))
    ...
    for model_name in [args.model]:
        ...
        perf_flipped = np.flipud(performance)
        num_layers = performance.shape[0]
        num_heads = performance.shape[1]
        ...
        best_head = max_idx[1]
        print(f"\nBest {args.axis} performance: {max_perf:.3f} at Layer {best_layer}, Head {best_head}")
        ...
        plt.title(f"Spearman Performance for {model_name} ({args.axis} Prediction {args.prob} {suffix})")
        plt.xlabel("Attention Head")
        plt.ylabel("Layer")
